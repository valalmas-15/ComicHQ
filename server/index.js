...
// Auto-init schema (SQLite Version)
function initSchema() {
  db.serialize(() => {
    // 👤 Users table
    db.run(`
    ...
    const migrate = (table, column, definition) => {
      // Use standard definition for ALTER TABLE (SQLite has limits on complex defaults in ALTER)
      const baseDefinition = definition.split('DEFAULT')[0];
      db.run(`ALTER TABLE ${table} ADD COLUMN ${column} ${baseDefinition}`, (err) => {
        if (!err) {
            console.log(`✅ Added ${column} to ${table}`);
            // If it's a timestamp, sync with added_at
            if (column === 'updated_at') {
                db.run(`UPDATE ${table} SET updated_at = added_at WHERE updated_at IS NULL`);
            }
        }
      });
    };

    migrate("manga_library", "type", "TEXT");
    migrate("manga_library", "has_update", "BOOLEAN");
    migrate("manga_library", "last_chapter_count", "INTEGER");
    migrate("manga_library", "updated_at", "DATETIME");
    migrate("user_library", "added_at", "DATETIME");

    // 🔄 Ensure History is Multi-User (Old migration)
    db.get("SELECT sql FROM sqlite_master WHERE type='table' AND name='reading_history'", (err, row) => {
      if (row && !row.sql.includes("user_id")) {
        console.log("⚠️ Old DB detected! Migrating history to multi-user format...");
        db.serialize(() => {
          db.run("DROP TABLE IF EXISTS reading_history");
          db.run("DROP TABLE IF EXISTS read_chapters");
          // Re-running initSchema isn't possible here nicely, but tables will be created correctly on next start
          console.log("⚠️ Tables dropped. Please restart or wait for auto-init next run.");
        });
      }
    });
...
app.post("/api/sources-ping/:provider", async (req, res) => {
  ...
  try {
    ...
    db.run(
      ...
      (err) => {
        ...
        res.json({
          success: true,
          message: `Ping success for ${providerName}`,
        });
      },
    );
  } catch (error) {
    ...
    db.run(
      ...
      (err) => {
        res.json({
          success: false,
          message: `Ping failed for ${providerName}`,
          error: errStr,
        });
      },
    );
...
app.get("/api/chapters", async (req, res) => {
  ...
  try {
    ...
    if (!chapters || chapters.length === 0) {
      console.warn(`⚠️ No chapters extracted for: ${url}`);
    } else {
...
app.get("/api/proxy", async (req, res) => {
  ...
  try {
    ...
    // 🛡️ Special Referer rules for anti-hotlinking
    if (
      url.includes("mangabat") ||
      url.includes("manganelo") ||
      url.includes("manganato") ||
      url.includes("nhato") || // manganato mirrors
      url.includes("2xstorage") ||
      url.includes("mncdn")    // some mirrors
    ) {
    ...
    console.log(`Proxying: ${url.substring(0, 60)}... (With Referer: ${referer})`);
...
app.get("/api/pages", async (req, res) => {
  ...
  try {
    console.log(`Fetching pages for: ${url} (Provider: ${provider})`);
    const pages = await p.getPages(url);
    if (!pages || pages.length === 0) {
      console.warn(`⚠️ No pages found for this chapter!`);
    } else {
...
app.post("/api/library", authenticateToken, (req, res) => {
  ...
  console.log(`Adding to library: ${title} (${source_id}) for user ${userId}`);
  ...
  db.run(
    query,
    [title, source_id, provider, thumbnail_url, type || "manga"],
    function (err) {
      if (err) {
        console.error("Manga Library Insert Error:", err);
        return res.status(500).json({ error: `Failed to add to database: ${err.message}` });
      }

      db.get(
        "SELECT id FROM manga_library WHERE source_id = ?",
        [source_id],
        (err, row) => {
          if (err || !row) {
            console.error("Retrieval Error:", err);
            return res.status(500).json({ error: "Failed to confirm entry" });
          }
          ...
          db.run(libQuery, [userId, row.id], (err) => {
            if (err) {
              ...
              return res.status(500).json({ error: "Failed to link user library" });
            }
            console.log(`Success: Linked ${title} to user ${userId}`);
...
app.delete("/api/library/:id", authenticateToken, (req, res) => {
  db.run(
    "DELETE FROM user_library WHERE user_id = ? AND manga_id = ?",
...
// 🕒 Reading History & Progress (SQLite Version)
app.post("/api/history", authenticateToken, (req, res) => {
  ...
  const saveHistory = (mId) => {
    ...
    `;
    db.run(
      "INSERT OR IGNORE INTO read_chapters (user_id, manga_id, chapter_id) VALUES (?, ?, ?)",
      [userId, mId, chapter_id],
    );
    db.run(
      query,
      [userId, mId, chapter_id, chapter_title, last_page],
      (err) => {
        if (err)
          return res
            .status(500)
            .json({ error: `Failed to update history: ${err.message}` });
        res.json({ success: true, manga_id: mId });
      },
    );
  };

  if (!manga_id && source_id && manga_title && provider) {
    // Auto-add to generic library first (but NOT user_library automatically unless they bookmark it)
    const libQuery = `
      INSERT INTO manga_library (title, source_id, provider, thumbnail_url, type) 
      VALUES (?, ?, ?, ?, ?) 
      ON CONFLICT (source_id) DO UPDATE SET added_at = CURRENT_TIMESTAMP
    `;
    db.run(
      libQuery,
      [manga_title, source_id, provider, thumbnail_url || "", type || "manga"],
      function (err) {
        if (err)
          return res
            .status(500)
            .json({ error: "Failed to auto-add to generic library" });
        ...
        db.get(
          ...
          (err, row) => {
            if (err || !row)
              return res
                .status(500)
                .json({ error: "Failed to retrieve auto-added library id" });
            saveHistory(row.id);
          },
        );
      },
    ...
    res
      .status(400)
      .json({ error: "Missing manga_id or sufficient details to auto-add" });
  }
});

// Bulk mark as read/unread
app.post("/api/history/bulk", authenticateToken, (req, res) => {
  const { manga_id, chapter_ids, action } = req.body; // action: 'read' or 'unread'
  if (!manga_id || !chapter_ids || !action)
  ...
  db.serialize(() => {
    ...
    chapter_ids.forEach((cid) => stmt.run(userId, manga_id, cid));
    stmt.finalize((err) => {
      if (err) return res.status(500).json({ error: "Bulk update failed" });
      res.json({ success: true });
    });
  });
});
...
app.get("/api/read-chapters/:manga_id", authenticateToken, (req, res) => {
  db.all(
    "SELECT chapter_id FROM read_chapters WHERE user_id = ? AND manga_id = ?",
    [req.user.id, req.params.manga_id],
    (err, rows) => {
      if (err)
...
app.post("/api/track/read", authenticateToken, (req, res) => {
  ...
  db.run(
    ...
    (err) => {
      if (err)
        return res.status(500).json({ error: "Failed to mark chapter read" });
      res.json({ success: true });
    },
  );
});
...
app.post("/api/track/unread", authenticateToken, (req, res) => {
  ...
  db.run(
    ...
    (err) => {
      if (err)
        return res.status(500).json({ error: "Failed to unmark chapter" });
      res.json({ success: true });
    },
  );
});
...
app.get("/api/history", authenticateToken, (req, res) => {
  const query = `
    SELECT rh.*, ml.title, ml.thumbnail_url, ml.provider, ml.source_id, ml.type
    FROM reading_history rh 
    JOIN manga_library ml ON rh.manga_id = ml.id 
    WHERE rh.user_id = ?
...
module.exports = { db }; // Export db for the worker
