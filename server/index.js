...
app.post("/api/history", authenticateToken, (req, res) => {
  ...
  if (!manga_id && source_id && manga_title && provider) {
    db.run("INSERT INTO manga_library (title, source_id, provider, thumbnail_url, type) VALUES (?, ?, ?, ?, ?) ON CONFLICT (source_id) DO UPDATE SET added_at = CURRENT_TIMESTAMP", 
       [manga_title, source_id, provider, thumbnail_url || "", type || "manga"], function(err) {
      db.get("SELECT id FROM manga_library WHERE source_id = ?", [source_id], (err, row) => {
...
app.post("/api/history/bulk", authenticateToken, (req, res) => {
  ...
  if (!manga_id || !chapter_ids || !action) return res.status(400).json({ error: "Missing data" });

  const userId = req.user.id;
  db.serialize(() => {
    const stmt = action === "read"
        ? db.prepare("INSERT OR IGNORE INTO read_chapters (user_id, manga_id, chapter_id) VALUES (?, ?, ?)")
        : db.prepare("DELETE FROM read_chapters WHERE user_id = ? AND manga_id = ? AND chapter_id = ?");
...
app.get("/api/read-chapters/:manga_id", authenticateToken, (req, res) => {
  db.all("SELECT chapter_id FROM read_chapters WHERE user_id = ? AND manga_id = ?", [req.user.id, req.params.manga_id], (err, rows) => {
    if (err) return res.status(500).json({ error: "Failed to fetch read list" });
