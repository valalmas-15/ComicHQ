...
function initSchema() {
  db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS manga_library (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, source_id TEXT UNIQUE NOT NULL, provider TEXT NOT NULL, thumbnail_url TEXT, last_chapter_count INTEGER DEFAULT 0, latest_chapter_title TEXT, latest_chapter_id TEXT, latest_chapter_date TEXT, has_update BOOLEAN DEFAULT 0, added_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, type TEXT DEFAULT 'manga')`);
    db.run("ALTER TABLE manga_library ADD COLUMN latest_chapter_title TEXT", (err) => {});
    db.run("ALTER TABLE manga_library ADD COLUMN latest_chapter_id TEXT", (err) => {});
    db.run("ALTER TABLE manga_library ADD COLUMN latest_chapter_date TEXT", (err) => {});
